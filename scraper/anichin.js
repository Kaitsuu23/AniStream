...
async function getEpisode(id) {
  ...
  // Derive animeId: first try breadcrumb (2nd link = parent anime), fallback to regex
  let animeId = '';
  const breadcrumbLinks = $('nav.ts-breadcrumb a, .breadcrumb a, .ts-breadcrumb a').toArray();
  if (breadcrumbLinks.length >= 2) {
    const parentHref = $(breadcrumbLinks[1]).attr('href') || '';
    animeId = parentHref.replace(BASE_URL, '').replace(/^\//, '').replace(/\/$/, '');
  }
  if (!animeId) {
    animeId = id
      .replace(/-episode-\d+.*$/i, '')
